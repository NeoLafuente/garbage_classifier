...
def data_exploration_tab():
    ...
    cached_dir = os.path.join(os.getcwd(), "cached_data")
    ...
    def show_random_samples():
        eda.plot_random_examples_per_class()
        return "✅ Random samples plotted."

    def show_mean_prototypes():
        eda.plot_mean_images_per_class(filename=mean_arrays_path)
        return f"✅ Mean prototypes plotted (saved in {mean_arrays_path})"

    def show_median_prototypes():
        eda.plot_median_images_per_class(filename=median_arrays_path)
        return f"✅ Median prototypes plotted (saved in {median_arrays_path})"

    def show_class_distribution():
        eda.plot_class_distribution()
        return "✅ Class distribution plotted."
    ...
    btn_random.click(fn=show_random_samples, outputs=output_text)
    btn_distribution.click(fn=show_class_distribution, outputs=output_text)
    btn_mean.click(fn=show_mean_prototypes, outputs=output_text)
    btn_median.click(fn=show_median_prototypes, outputs=output_text)

    return [btn_random, btn_distribution, btn_mean, btn_median, output_text]
